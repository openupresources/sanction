    ...
    module For
      ...
      module ClassMethods
        def self.extended(base)
          base.scope :for_scope_method, lambda {|*args|
            if args.include? Sanction::Role::Definition::ANY_TOKEN
              where("#{ROLE_ALIAS}.principal_type IS NOT NULL")
            else
              ...
              args.each do |arg|
                if arg.is_a? Class
                  collection.where! ["#{ROLE_ALIAS}.principal_type = ?", arg.to_s]
                else
                  collection.where! ["#{ROLE_ALIAS}.principal_type = ? AND (#{ROLE_ALIAS}.principal_id = ? OR #{ROLE_ALIAS}.principal_id IS NULL)", arg.class.name.to_s, arg.id]
                end
              end
              collection
            end
